...
public class WebSocketServerHandler extends SimpleChannelInboundHandler<Object> {
    ...
    private void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest req) {
        ...
        ChannelMap.instance().put(Long.parseLong(sessionId), ctx.channel());
    }

    private String getSessionId(String uri) {
        try {
            String[] pathAndParms = uri.split("\\?", 2);
            String[] params = pathAndParms[1].split("\\&");
            for (String param: params) {
                String[] keyval = param.split("=", 2);
                if (keyval.length < 2) {
                    continue;
                } else {
                    if ("sessionId".equals(keyval[0])) {
                        return keyval[1];
                    } else {
                        continue;
                    }
    ...
    private void handleWebSocketFrame(ChannelHandlerContext ctx, WebSocketFrame frame) {
        ...
        MessageManager.instance().add(new Data(request));
